...
case $menuitem in
	...
	;;
	Cinnamon) 
	 echo "Scanning CINNAMON Environment [Please wait...]";
	 sudo ./lsdumper $cinnamon | grep 'file:///home/' >> /tmp/cinn.txt;
	 sudo ./lsdumper $cinnamon | grep 'nemo-icon-position-timestamp' >> /tmp/cinn.txt;
	;;
	Skip) 
	 echo "Skipping..."; 
	sleep 2;
	...
	if [[ -f "/tmp/cinn.txt" ]];
	then
	    zenity --text-info --filename=/tmp/cinn.txt;
	fi
